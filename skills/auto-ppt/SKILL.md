...
接收到 PPT 生成请求后，**严格按照以下6个阶段顺序执行**，不可跳步：
...
**1. 本地保存**
- 文件路径：`/home/ubuntu/.openclaw/workspace/auto-ppt-outputs/presentation.pptx`
- 用户可自行复制或移动文件

**2. GitHub 上传**（需用户在 TOOLS.md 中配置）
- 配置 GitHub Token、仓库和上传目录
- 上传后清理本地文件
...
文件位置：/home/ubuntu/.openclaw/workspace/auto-ppt-outputs/presentation.pptx

请选择交付方式：
1. 本地保存（文件保留在原位置）
2. GitHub 上传（需在 TOOLS.md 中配置 Token 和仓库信息）
3. 其他方式（请指定）
```
