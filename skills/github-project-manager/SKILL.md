...
IF 仓库下没有 project
  → 自动创建默认 project
  → project name 设为 "default"，其余设置保持默认

IF 仓库下只有一个 project
  → 直接使用该 project，继续 STEP 4

IF 仓库下有多个 project 且无法从上下文判断归属
  → 列出所有 project 供用户选择
  → 用户确认后继续 STEP 4
```
...
```bash
# 创建默认 project（如需要）
gh project create --owner "$OWNER" --title "default"
```

确认 project 后，将 project_id 写入 `jobs.json` 对应作业的 `config.project_id`（如为定时作业）。
